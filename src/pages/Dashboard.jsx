    ...
    useEffect(() => {
        const fetchProperties = async () => {
            try {
                ...
                setProperties(propertyList);
                console.log(propertyList, "Fetched properties"); // Debugging output
            } catch (error) {
                console.error("Error fetching properties:", error);
            }
        };
    ...
    useEffect(() => {
        ...
        if (checkInDate && checkOutDate) {
            ...
            filteredList = filteredList.filter(property => {
                const propertyCheckIn = property.checkIn ? new Date(property.checkIn) : null;
                const propertyCheckOut = property.checkOut ? new Date(property.checkOut) : null;
                return propertyCheckIn && propertyCheckOut && propertyCheckIn <= checkIn && propertyCheckOut >= checkOut;
            });
        }

        const totalGuests = (adults || 0) + (children || 0);
        if (totalGuests > 0) {
            filteredList = filteredList.filter(property => property.maxGuests >= totalGuests);
        }

        setSearchResults(filteredList);
    }, [destination, checkInDate, checkOutDate, adults, children, properties]);

    // ** Apply Category Filter on Top of Search Results **
    const filteredByCategory = searchResults.filter(property =>
        category
        ...
        const sortedProperties = [...filteredByCategory].sort((a, b) => {
        ...
        <Box p={5} maxW="90%" mx="auto">
            ...
            <Box>
                <Flex gap={10} color="gray.500" flexDirection="row" m="20px 0px 20px 40px"
                    ...
                    {/* <label For="">Sort by Price</label> */}
                    <Flex gap={10} color="gray.500" justifyContent={"center"}>
                        <select onChange={(e) => setSortOrder(e.target.value)} value={sortOrder} style={{ border: "1px solid gray", padding: "5px", borderRadius: "5px" }}>
                            <option value="">Sort by Price</option>
                ...
                <SimpleGrid columns={[1, 2, 3, 4]} gap={5} height="600px">
                    {sortedProperties.map((property) => (
                        <Box key={property.id} p={4} borderWidth="1px" borderRadius="md">
                            ...
                            <Heading size="md">{property.title}</Heading>
                            <Text>{property.location}</Text>
                            <Text>Price: ₹{property.price} / night</Text>
                            <Link to={`/property/${property.id}`}>
                                <Button mt={2}>View Details</Button>
                            </Link>
                        </Box>
